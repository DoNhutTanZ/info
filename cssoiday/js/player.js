(function ($) {
  ...
  var isShowNotification = false,
    ...
    isShuffle =
      localStorage.qplayer == undefined
        ? false
        : localStorage.qplayer === "true"
        ? true
        : false;
  ...
  for (var i = 0; i < playlist.length; i++) {
    var item = playlist[i];
    $("#playlist").append(
      '<li class="lib" style="overflow:hidden;"><strong style="margin-left: 5px;">' +
        item.title +
        '</strong><span style="float: right;" class="artist">' +
        item.artist +
        "</span></li>"
    );
    if (item.mp3 == "") {
  ...
  if (
    isShuffle &&
    shuffle_array != undefined &&
    playlist.length === (shuffleArray = JSON.parse(shuffle_array)).length
  ) {
  ...
  var play = function () {
    ...
    if (isRotate) {
      $("#player .cover img").css(
        "animation",
        "9.8s linear 0s normal none infinite rotate"
      );
  ...
  var beforeLoad = function () {
    var endVal =
      this.seekable && this.seekable.length ? this.seekable.end(0) : 0;
  };
  ...
  var loadMusic = function (i) {
    ...
    var newaudio = $("<audio>")
      .html('<source src="' + item.mp3 + '"><source src="' + item.ogg + '">')
      .appendTo("#player");
    $(".cover").html('<img src="' + item.cover + '" alt="' + item.album + '">');
    $(".musicTag").html(
      "<strong>" +
        item.title +
        '</strong><span> - </span><span class="artist">' +
        item.artist +
        "</span>"
    );
  ...
  $("#QPlayer .ssBtn").on("click", function () {
    ...
    if ($(".ssBtn .adf").hasClass("on") === false) {
      if (isFirstPlay) {
        setTimeout(
          "showTips('#player .cover','Bấm vào bìa để mở (đóng) chơi ngẫu nhiên', " +
            function () {
              setTimeout(
                "showTips('#player .ctrl .musicTag','Nhấp và kéo thanh tiêu đề để tua nhanh (tua lại)')",
                1000
              );
            } +
            ");",
...
function isExceedTag() {
  ...
  if (
    $(".musicTag strong").width() +
      $(".musicTag span").width() +
      $(".musicTag .artist").width() >
    $(".musicTag").width()
  ) {
    isExceedTag = true;
  }
...
function showNotification(info) {
  ...
  $("body").append(
    '<div class="qplayer-notification animation-target"><span class="qplayer-notification-icon">i</span><span class="body" style="box-shadow: rgba(0, 0, 0, 0.0980392) 0px 0px 5px;"><span class="message"></span></span><a class="close" href="#" onclick="closeNotification();return false;">×</a><div style="clear: both"></div>'
  );
  $(".qplayer-notification .message").text(info);
  //用width:auto来自动获取通知栏宽度
  var width =
    $(".qplayer-notification").css({ opacity: "0", width: "auto" }).width() +
    20;
  ...
  autoClearTimer = setTimeout(
    "if ($('.qplayer-notification').length>0) {closeNotification()}",
    8000
  );
}
...
function showTips(div, info, func) {
  var box_height = 100;
  $("body").append(
    '<div class="qplayer_tips"><span class="tips_arrow"></span><span class="info" style="display:none">' +
      info +
      '</span><button class="tips_button" onclick="removeTips()">Continued</button></div>'
  );
  $(".qplayer_tips").css({
